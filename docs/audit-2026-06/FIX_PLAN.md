...
| # | 任务 | 级别 | 状态 | 工作量 | 凭证 |
|---|------|------|------|--------|------|
| 1 | 修冷启动白屏（Neon 暂停 + SSR 注水 0） | 🔴 P0 | ✅ 代码已上线，production smoke 已过 | 0.5d | `ResearcherDirectory.tsx` / `app/page.tsx` / Vercel `dpl_3gS5j1pjLMTHTRVmC3a45PTxTmLy` |
| 2 | 修 "OpenAI基金会" 全库错译 | 🔴 P0 | ✅ 已清除（含 Tesla / 开放人工智能基金会残留） | 0.5d | DB 复核 0 残留 |
| 3 | 机构去重合并 | 🔴 P0 | ✅ safe 桶 + 两批 review 已执行 / 🧭 边界已裁定保留分开 | — | Organization 613 / `org_review_after_second_decisions.json` |
...
| 11 | career 规范化（currentTitle 错位） | 🟡 P2 | ✅ 泛化 role / currentTitle mismatch 均已清空 | — | duplicate org/role/position-like/vague/currentTitle mismatch 归零 |
...
- **生产部署（2026-06-10）**：Vercel production deployment `dpl_3gS5j1pjLMTHTRVmC3a45PTxTmLy` 为 READY，`ai-person-agent.vercel.app` 与 FC 自定义域 `people.llmxy.xyz` 首页均返回 200 且 etag 同为 `a6d7e980b0601aa6f07292c2e446c3e6`；两域名 `/api/person/directory?limit=3` 均返回 `pagination.total=230`、`stats.totalPeople=230`。
- **凭证**：`npm run build` / `npm run deploy:build` 通过；production smoke 验证首页和 directory API 均为 200，首页 HTML 仍由 Next prerender 正常输出。Neon 预热 / 付费档仍是运维层优化，代码侧已先兜住错误体验。
...
- **review 第一批（2026-06-10）**：新增 `org_review_decisions_2026_06_10.json` 与 `apply_org_review_decisions.mjs`，已合并 Stanford University / Carnegie Mellon University / Meta / CAS ICT / Dalle Molle Institute / IBM / King's College, Cambridge 7 个确定性重复簇，9 条 PersonRole 转指到 canonical，7 个 People.organization 展示值同步。
- **review 第二批（2026-06-10）**：新增 `org_review_decisions_second_2026_06_10.json`，已合并 Stanford HAI 短名/全名重复、把 `Stanford University, Sequoia Professor` 转回 Stanford University，并统一 6 个 Facebook/FAIR/Meta Superintelligence Labs 展示标签；剩余 9 个边界簇明确保留分开。
- **当前状态**：Organization 总数 **613**；`career_normalization_audit_after_org_review_second.json` 复核 duplicateOrgClusters / duplicateRoleGroups / peopleOrganizationDuplicates / position-like Organization / vagueRoles / currentTitleOrgMismatches 均为 0。
- **保留边界**：Google / Google Brain / Google Research / Google Cloud / Google DeepMind / Alphabet 不是单纯重复，仍分开；NYU/CMU/Stanford 子机构、Twitter/X 历史名、Facebook/FAIR/Meta AI/MetaMind、IBM Research、UBC、King's College NZ 等已按展示口径裁定保留分开。
- **复核清单（2026-06-10）**：`org_review_after_second_decisions.json` 当前覆盖 9 个已裁定保留边界簇、36 条 Organization 候选、167 条 PersonRole 引用、14 个 People.organization 展示值；导出脚本已过滤合并后只剩单个 canonical 的簇和无引用机构噪声。
...
- **防回流** 🔧：relation needs_review 已归零；后续重点是守住 `relation-validation`，避免新抓取重新写入弱边。
...
- **最终落库**：`QAAuditLog` 初始全量重洗共 **4278 行**，最后写入 **2026-06-07 13:16:54（北京）**。candidate live audit 后，共审过 RawPoolItem 4307 条；safe prune 第一批后的 3688 条审计面是历史快照，当时分布为 keep 2178 / reject 1199 / review 311，后续 reject/review 尾巴已处理到 `reviewUnresolvedRows=0` / `dependencyRows=0`。
- **脏数据全景**：safe prune 前 keep 约 50%，明确 reject+empty+duplicate+incomplete 约 43%；把 review 也算入当时风险，则约 50%。safe prune 第一批已删除 duplicate 157 / empty_content / incomplete，后续 reject/review 尾巴已通过 strict bucket、manual prune、refetch、人工裁定和 keep 审计处理到 `reviewUnresolvedRows=0` / `dependencyRows=0`。
...
- **安全边界**：第一批 prune 只删除 latest QA verdict 为 `duplicate` / `empty_content` / `incomplete` 的 RawPoolItem 619 条；`QAAuditLog` 和 `Card` 未删除。`export_prune_candidates.ts` 当时重导出的 reject/review 候选已走完后续 strict bucket、manual prune、refetch 与 review manual apply；删除快照见 `prune_archive_safe.json`，执行摘要见 `prune_execution_safe.json`。
...
| 任务 | 状态 | 说明 |
|------|------|------|
| career 规范化 currentTitle | ✅ | 7 条早期修正（Mira Murati/苏姿丰/IBM CEO 等）后，`savePersonRoles()` 已补机构标准化、无 QID 查找/创建、无机构 P39 跳过、Employee 不覆盖具体职位；`apply_career_normalization_safe.ts --execute` 已合并 Cambricon Technologies / DeepSeek 重复机构，去重 Christopher Manning / Noam Shazeer 的 organization 数组，删除 Zuckerberg 1 条 exact duplicate role；`apply_career_review_safe_fixes.ts --execute` 已删除 1 个无 role / 无 People 引用的空 CTO Organization；`apply_career_review_decisions.ts --execute` 已把 CEO position-like 下 6 条 role 转回 You.com / Alphabet / Google / Microsoft，删除 Rasmussen 空泛 professor role，把 Elon Musk 在 OpenAI 的泛化 Employee 修为 Co-chair，删除 5 条 `Employee @ 董事会成员`，并删除 CEO/professor/board of directors member 三类 position-like Organization；随后把 Daniela Amodei / Ethan Mollick / Marc Benioff / Marian Croak / 唐杰 / 李莲等泛化 Employee 更新为具体职位，补正李开复在 Apple / CMU 的 2 条泛化履历，删除 Mira Murati @ Goldman Sachs 的冲突 Employee 行；又把 33 条 education generic role 按证据更新为学位/学习阶段，删除 3 条不可信教育噪声。`People.organization` 口径确认为“展示身份锚点 + 重要背书机构”，完整履历由 `PersonRole` 承载；`apply_current_title_decisions.ts --execute` 已按三批来源支持决策补正 currentTitle / organization；机构 review 两批后补正 Geoffrey Hinton 的 `Student @ King's College, Cambridge` 为 `Undergraduate Student`，并修正李飞飞 Stanford HAI / Sequoia Professor 机构归属。复核：Organization 613 / PersonRole 1114 / People 230；duplicate org clusters 0 / duplicate role groups 0 / position-like org 0 / vague roles 0 / People.organization 重复 0 / currentTitle mismatch 0。 |
...
| 卡片有序聚合重生成 | ✅ | `scripts/enrich/regenerate_cards.ts --execute` 已执行，默认只使用最新 QAAuditLog keep 过滤 RawPoolItem；ready 人群新增 327 张卡、覆盖 59 人，candidate 人群追加 41 张卡、11 人均有 5-7 张卡。脚本已加 `--min-items=3` 薄输入保护、`--candidates-only` 候选人模式、topN 保存上限，并删除 18 张薄输入误生成卡，复核 `thin=[]`。2026-06-10 已补 generation-based 展示替换：`Card.generationId/isActive/archivedAt` 已加并执行 DB additive ALTER，前台只展示激活卡，重聚合默认 archive-active，旧卡保留历史 |
...
这几项已完成本轮裁定，不再作为旧数据清洗遗留：

1. **prune reject/review 尾巴** — safe batch、hard-tail manual、第二批 strict reject 和十六批 manual prune 已执行；515 条 refetch 队列已全量完成：Tavily advanced + MiMo 二十六批完成，人工 curation 后 345 个 source 已 additive 写入/确认，其中新增 205 条 RawPoolItem + 205 条 QAAuditLog keep；refetch 后原始 `reject` 的 96 条 unresolved 已做人工裁定，89 条低质/错挂 RawPoolItem 已删除并写入 `manual_prune_tail_after_refetch` reject 审计。当时 7 条展示依赖已继续处置：4 张 active card 归档、1 张 active card 换源、2 条 GitHub profile 坏抓取修复为 keep，并删除 5 条旧 reject RawPoolItem。原始 `review` unresolved 124 条已全部裁定完成：删除 no-good / low-info / thin-mismatch / residual-low-value / low-context / display-link cleanup 行，修复 7 条 GitHub repo 短抓取，保留 29 条 latest keep，移除 Alec 空主页 officialLink。当前 `reviewUnresolvedRows=0`、`dependencyRows=0`。旧 refetch 队列已处理完，不扩大自动删除。
2. **机构边界剩余** — 两批已裁定，剩余 NYU/CMU/Stanford 子机构、Twitter/X 历史名、Facebook/FAIR/Meta AI/MetaMind、IBM Research、UBC、King's College NZ 等按展示口径保留分开；后续只防止新抓取重新写入重复或职位型机构。

---

## 六、未来防回流顺序

1. **防回流总关卡**：已有 `npm run audit:content-guard` 只读检查 prune unresolved、career normalization、relation needs_review、card reaggregation 和保守改写队列；新增数据写入后优先跑 `npm run audit:post-ingest-guard`，它会先刷新轻量审计产物再调用总关卡。
2. **新人执行前关卡**：新增人选或新人资料补全执行前跑 `npm run audit:newcomer-preflight`；入口 checklist 与阈值见 `CANDIDATE_INTAKE_POLICY.md` / `CONTENT_REVIEW_POLICY.json`。
3. **prune reject/review 尾巴**：refetch 队列和原始 reject/review unresolved 都已清零；下一步不再围绕旧 sourceQueries 批量重抓，新抓取后由 guard 发现回升再开小批 review。
4. **机构边界防回流**：剩余边界已裁定保留分开，后续新抓取只合并确定性重复机构，不再把历史名、实验室和院系压成母机构。
5. **relation / career / card 新增数据防回流**：现有 needs_review、currentTitle mismatch 和卡片展示替换机制均已闭环；后续只需在新抓取/导入后复跑 guard、review bucket 和 generation dry-run。
