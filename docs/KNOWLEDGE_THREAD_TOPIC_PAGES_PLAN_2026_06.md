...
| 脚本 | 作用 |
|---|---|
| `scripts/knowledge/seed_loop_engineering_thread.ts` | Loop Engineering dry-run seed 骨架 |
| `scripts/knowledge/fetch_official_sources.mjs` | 抓官方博客、docs、changelog、RSS |
| `scripts/knowledge/fetch_youtube_transcripts.mjs` | 按视频 URL 或本地字幕目录抓字幕候选 |
| `scripts/knowledge/fetch_paper_sources.mjs` | 抓 OpenAlex / DOI / URL 论文候选 |
| `scripts/knowledge/fetch_implementation_sources.mjs` | 抓 GitHub、examples、docs snippets 等工程落地来源 |
| `scripts/knowledge/review_thread_pack.mjs` | 输出 Mimo 可复核包 |

财报抓取如果要做，应放到公司 / 机构页链路，例如 `scripts/company/fetch_company_financial_sources.mjs`，不放进知识主题页 P0。
