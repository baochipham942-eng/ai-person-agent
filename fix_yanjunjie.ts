...
async function main() {
    for (const update of X_UPDATES) {
        console.log(`处理 ${update.name}...`);
        ...
        console.log(`  已更新X链接: @${update.handle}`);

        // 下载头像
        const filename = `${person.id}.jpg`;
        const filepath = path.join(AVATAR_DIR, filename);
        const avatarUrl = `https://unavatar.io/twitter/${update.handle}`;
