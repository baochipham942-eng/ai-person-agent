...
const MISSING_VIPS = [
    { name: 'Daniel Gross', handle: 'danielgross', keywords: ['investor', 'ai', 'y combinator'] },
    { name: 'Lukasz Kaiser', handle: 'lukaszkaiser', keywords: ['transformer', 'google', 'openai'] },
    { name: 'Wojciech Zaremba', handle: 'woj_zaremba', keywords: ['openai', 'cofounder'] }
];
...
async function main() {
    for (const vip of MISSING_VIPS) {
        ...
        if (!person) {
            ...
            const results = await searchWikidata(vip.name, 5);

            let targetQid: string | null = null;

            if (results.length > 0) {
                // Simple heuristic to pick the right one if multiple
                // For now, just pick the first one, or maybe log them?
                // Given these are famous, first is likely correct.
                console.log(`  Found ${results.length} results. Using first: ${results[0].label} (${results[0].id}) - ${results[0].description}`);
                targetQid = results[0].id;
            } else {
            ...
            person = await prisma.people.create({
                data: {
                    qid: entity.qid,
                    name: entity.label, // Use label from Wikidata
                    aliases: entity.aliases,
                    description: entity.description,
                    avatarUrl: null, // Will fetch below
