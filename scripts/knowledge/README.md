...
```bash
pnpm knowledge:preflight -- --output=/tmp/knowledge-thread-preflight.json
```

By default it checks the Loop Engineering and Agentic Coding reviewed packs, analyzes the local `KnowledgeThread` migration SQL, and does not query the database. After `DATABASE_URL` points to a confirmed local/dev/staging database, add `--check-db`:

```bash
pnpm knowledge:preflight -- --check-db --output=/tmp/knowledge-thread-preflight-db.json
```
