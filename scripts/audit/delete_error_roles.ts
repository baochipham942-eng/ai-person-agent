...
async function main() {
  ...
  const targets: { roleId: string; person: string; org: string; role: string; conf: number; note: string }[] = [];
  ...
  const backupPath = path.join(process.cwd(), 'data/audit/deleted-error-roles-backup.json');
  fs.writeFileSync(backupPath, JSON.stringify(backup, null, 2));
  console.log(`📦 已备份 ${backup.length} 条完整行 → ${backupPath}`);
